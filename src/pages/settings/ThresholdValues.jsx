...
import { fetchSensorsByDeviceThreshold } from '../../api/sensorListApi';
...
const ThresholdValues = () => {
  ...
  const fetchDependentData = async (threshold) => {
    try {
      ...
      if (deviceId) {
        const fetchedSensors = await fetchSensorsByDeviceThreshold(deviceId); // fixed
        setSensors(fetchedSensors); // no frontend filter needed
      }
    } catch (err) {
      console.error("Error fetching dependent data for edit:", err);
    }
  ...
  const handleDeviceChange = async (e) => {
    ...
    if (deviceId) {
      try {
        const allSensors = await fetchSensorsByDeviceThreshold(deviceId);
        const filteredSensors = allSensors.filter(sensor => sensor.trigger_type_id === 1);
        setSensors(filteredSensors);
      } catch (err) {
