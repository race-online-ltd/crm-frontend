...
const Home = () => {
  ...
  // selected id clean
  const cleanedSelected = sanitize(dataCenterId);

  // যদি selected invalid হয় → fallback to user allowed
  if (!cleanedSelected) {
  ...
  // ============================================================
  useEffect(() => {
      if (!effectiveDataCenterIds || effectiveDataCenterIds.length === 0) return;
      ...
      const enriched = {
        sensor_types: sensorTypes.map((sensorType) => ({
          ...
          sensors: sensorType.sensors.map((sensor) => {
            ...
            return { ...sensor, thresholds: matchedThresholds, status: derivedStatus };
          }),
        })),
      };
...
useEffect(() => {
  ...
  // 🔥 NEW ALARM: sensor_id নেই prev-এ
// ✅ এইটা করো
const newAlarms = alarmData.filter(
  (item) => !prev.some((p) => p.sensor_id === item.sensor_id && p.value === item.value)
);

  if (newAlarms.length > 0) {
    console.log("🆕 INSERT:", newAlarms);
    storeSensorFault({ sensors: newAlarms });
  }

  // 🔥 CLEARED: prev-এ ছিল, এখন নেই → ref থেকে সরাও, DB-তে কিছু করো না
  const clearedIds = prev
    .filter((p) => !alarmData.some((item) => item.sensor_id === p.sensor_id))
    .map((p) => p.sensor_id);

  if (clearedIds.length > 0) {
    console.log("✅ CLEARED (no DB insert):", clearedIds);
  }

  // 🔥 ALWAYS UPDATE — শুধু active alarms রাখো
  prevAlarmRef.current = alarmData;
